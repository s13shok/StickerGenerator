...
namespace Sticker_Generator
{
    public partial class MainForm : MaterialForm
    {
        ...
        public MainForm()
        {
            ...
            //materialSkinManager.ColorScheme = new ColorScheme(Primary.BlueGrey500, Primary.BlueGrey100, Primary.DeepOrange100, Accent.Blue700, TextShade.WHITE);
            coef = 10;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            label2.Text = BitConverter.ToString(bytes);
            string hexVal = label1.Text;
            int[] numbers = new int[4];
            string[] s = new string[4];
            for (int i = 0; i < numbers.Length; i++)
            {
                s[i] = hexVal.Substring(i * 8, 8);
                numbers[i] = Convert.ToInt32(s[i], 16);
            }
            textBox1.Text = String.Join("  ", s);
            //textBox1.Text = sb.ToString().Replace(' ', '\n');

            label3.Text = numbers[0].ToString();
            label4.Text = numbers[1].ToString();
            label5.Text = numbers[2].ToString();
            label6.Text = numbers[3].ToString();
            long x1 = numbers[0];
            long x2 = numbers[1];
            long x = x1 + x2;
            long y1 = numbers[2];
            ...
            long z2 = y;
            long z = z2 + z1;
            var num = z.ToString("X10");
            MessageBox.Show(num);
            bytes = new byte[5];
            for (int i = 0; i < bytes.Length; i++)
            {
                bytes[i] = Convert.ToByte(num.Substring(i * 2, 2), 16);
            }
