...
namespace Sticker_Generator
{
    ...
    {
        public static bool[] CreateMD5(string input)
        {
            using (System.Security.Cryptography.MD5 md5 = System.Security.Cryptography.MD5.Create())
            {
                byte[] inputBytes = Encoding.ASCII.GetBytes(input);
                byte[] hashBytes = md5.ComputeHash(inputBytes).Reverse().ToArray();
                ulong firstPart = (ulong)BitConverter.ToUInt32(hashBytes, 0) + (ulong)BitConverter.ToUInt32(hashBytes, 4);
                ulong secondPart = (ulong)BitConverter.ToUInt32(hashBytes, 8) + (ulong)BitConverter.ToUInt32(hashBytes, 12);
                ulong sum = firstPart + secondPart;
                return CreateBoolArray(sum);
            }
        }

        public static bool[] CreateCRC64(string input)
        {
            byte[] inputBytes = Encoding.ASCII.GetBytes(input);
            ulong crc = 0;
            for (int i = 0; i < inputBytes.Length; i++)
            {
                crc = Table[(byte)(crc ^ inputBytes[i])] ^ (crc >> 8);
            }
            return CreateBoolArray(crc);
        }

        private static bool[] CreateBoolArray(ulong value)
        {
            BitArray bits = new BitArray(BitConverter.GetBytes(value));
            bool[] hashBits = new bool[35];
            for (int i = 0; i < hashBits.Length; i++)
            {
                hashBits[i] = bits[i];
            }
            hashBits = hashBits.Reverse().ToArray();
            return hashBits;
        }
